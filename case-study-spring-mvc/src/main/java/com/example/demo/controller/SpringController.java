...
public class SpringController {
	...
	public String submitForm(@Valid @ModelAttribute("command") Donor donor,BindingResult result) {
		...
		if(result.hasErrors()) {
			...
			template.postForObject("http://localhost:4012/api/v1/donors", requestBody, Donor.class);
		}
		return nextStep;
	}
	@GetMapping(path = "/getAllDonors")
	public String findAll(Model model) {
	Donor[] resp =template.getForObject("http://localhost:4012/api/v1/donors",Donor[].class);
	...
	public String submitFormCamp(@Valid @ModelAttribute("command") DonationCamp donation,BindingResult result) {
		...
		if(result.hasErrors()) {
			...
			template.postForObject("http://localhost:4014/api/v1/donations", requestBody, DonationCamp.class);
		}
		return nextStep;
	}
	...
	DonationCamp[] resp =template.getForObject("http://localhost:4014/api/v1/donations",DonationCamp[].class);
	...
	Donor[] resp =template.getForObject("http://localhost:4012/api/v1/donors/searchbybloodgroup/"+searchbybloodgroup,Donor[].class);
	model.addAttribute("data",resp);
	return "showDonors";
	...
	Donor[] resp =template.getForObject("http://localhost:4012/api/v1/donors/searchbycity/"+searchbycity,Donor[].class);
	model.addAttribute("data",resp);
	return "showDonors";
	...
	{
	template.delete("http://localhost:4012/api/v1/donors/"+value,Donor[].class);
	return "deletesuccess";
	}
	
	@GetMapping(path = "/getByDate")
	public String findCamps(Model model,String searchbyDate) {
	DonationCamp[] resp =template.getForObject("http://localhost:4014/api/v1/donations/searchbydate/"+LocalDate.parse(searchbyDate),DonationCamp[].class);
	model.addAttribute("details",resp);
	return "CampDetails";
	...
	{	
	template.put("http://localhost:4012/api/v1/donors/"+id+"/"+LocalDate.parse(updateDate),Donor[].class);
